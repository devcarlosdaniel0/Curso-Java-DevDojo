...
class Worker implements Runnable {
    ...
    public void run() {
        lock.lock();
        try {
            System.out.printf("Thread %s entrou no lock%n", name);
            System.out.printf("%d Threads esperando%n", lock.getQueueLength());
            System.out.printf("Thread %s vai dormir por 5 seg%n", name);
            Thread.sleep(5000);
